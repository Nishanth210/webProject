        ...
        <>
            <div className="space-x-3 py-2 sticky z-30 flex justify-between items-center w-full bg-slate-50 shadow-lg  ">
                <div className="flex gap-16 font-semibold ">
                    ...
                    <button className="hover:shadow-md px-3 rounded-md hover:bg-blue-400 hover:text-white transition ease-in-out" onClick={() => nav("/")}>
                        Home
                    </button>
                </div>
            ...
            <Transition as={Fragment} enter="transition ease-out duration-100" enterFrom="transform opacity-0 scale-95" enterTo="transform opacity-100 scale-100" leave="transition ease-in duration-75" leaveFrom="transform opacity-100 scale-100" leaveTo="transform opacity-0 scale-95">
                <Menu.Items className="absolute right-0 z-10 mt-5 w-36  origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <div className="py-1 ">
                        ...
                        <Menu.Item>
                            {({ active }) => (
                                <button onClick={() => nav("/equipment_review")} className={classNames(active ? "bg-gray-100 text-gray-900" : "text-gray-700", "block px-4 py-2 text-sm w-full")}>
                                    {" "}
                                    Equipments Review
                                </button>
                            )}
